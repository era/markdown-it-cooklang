...
for (let part in COOKLANG) {
  ...
  COOKLANG[part].placeholder = new RegExp(
    `^\\s*(\\[\\[${COOKLANG[part].plural}\\]\\]|\\[${COOKLANG[part].plural}\\]|\\$\\{${COOKLANG[part].plural}\\})\\s*$`,
    "ig"
  );
}
...
function render_cooklang_list(tokens, idx, options, env, slf, part) {
  ...
  if (list.length) {
    for (let item of list) {
      ...
      render += `<span class="${part.singular}">${markdownIt.renderer.render(
        item.children
      )}</span>`;
      render += `</li>`;
    }
...
function cooklang_inline(state, silent, part) {
  ...
  if (state.src.charCodeAt(start) !== part.startWith.charCodeAt(0)) {
    return false;
  }
  if (
    start > 0 &&
    ...
    ) /*there must be whitespace before the starting identifier*/
  ) {
    return false;
  }
...
module.exports = function cooklang_plugin(md) {
  ...
  md.inline.ruler.after(
    "html_inline",
    "cooklang-inline-ingredient",
    cooklang_inline_ingredient
  );
  md.renderer.rules.cooklang_inline_ingredient =
    render_cooklang_inline_ingredient;

  md.block.ruler.after(
    "html_block",
    "cooklang-ingredients",
    cooklang_ingredients
  );
  md.renderer.rules.cooklang_ingredients_open =
    render_cooklang_ingredients_open;
  ...
  md.inline.ruler.after(
    "html_inline",
    "cooklang-inline-cookware",
    cooklang_inline_cookware
  );
